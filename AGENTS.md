...
| Dirtiness / AFFECTED-set detection, and the WARM FOLD ITSELF (`build`, `affectedSet`, `warmOverride`, `warmResolve`) | `gen-memo` — the incremental plane, which the rebuilder core retired into. The warm fold and the override cone live there too, and the memo context the cold fold used to build is now composed by a caller holding both libraries: the cells that composed it here were the ecosystem's only site FORCING `build`, and they re-sited to `gen-memo`'s own suite with the retirement of `project`/`edges` — the plane is no longer an input of this repository's CI |
| Reading a resolved value off a context, and reading a node's dependency list | `gen-scope` — the evaluator publishes both itself. `project` was a currying re-spelling of the facade's `get` (`ctx.eval.facade.get id attr`) and owed no successor construct; `edges` was a projection of the seal's `trace.<id>.deps`. ★ The relation the seal publishes is the normalized UNION of the declared and structural halves, not the declared one alone, and it REFUSES an id the fold never saw where `edges` answered `[ ]` |
...
**Cold fold — NOT HERE ANY MORE.** It is `gen-scope`'s `foldEquations`, whose argument set is `{ scope; parseParent; schedule; declaredDependencies; settings ? { } }`. There is no `equations` formal — they are read off the schedule, so a caller cannot pair a schedule with equations it was not built from — and no `strataOrder` formal, because the order is `_scheduleWith`'s and is sealed nowhere. Build the schedule here, hand it there.

**Read-only consumers** — `lib/contract.nix`

| Export | Signature |
|---|---|
| `why` | `ctx -> { id; attr } -> [ { id; attr } ]` — the cross-product `declaredDependencies id` × `equations.<attr>.readsAttrs`, read through this module's internal `edges` binding |
...
| Fold everything into a context | `genScope.foldEquations { scope; parseParent; declaredDependencies; schedule = _scheduleWith { equations = eqs; }; }` |
| Use more than two strata | pass `strataOrder` to `_scheduleWith` (index 0 = the base graph-shaping stratum) |
| Read a resolved value | `ctx.eval.facade.get id attr` — `gen-scope`'s, not this library's |
| Ask which nodes an id depends on | `ctx.trace.<id>.deps` — `gen-scope`'s. It is the normalized UNION of the declared and structural relations, and it REFUSES an id the fold never saw rather than answering `[ ]` |
...
| Is the dynamic read-set recoverable? | `gen-scope`'s `lib/eval.nix` states it is only recoverable via `evalDebug`'s fresh-self-per-get, which defeats the memo, so the graph's edge set is the inspectable contract. That library declares no read set at all — the body is the read set, and `ctx.trace.<id>.deps` derives from `ctx.accessor.dependencies`, the normalized union of the declared and structural relations |
...
| `why` reports only **cross-node** pairs — it never surfaces an intra-node read, and returns `[ ]` for an unknown attr rather than throwing | `child` declares an edge to `parent`; `why ctx { id = "child"; attr = "derived"; }` ⇒ `[ { id = "parent"; attr = "self-v"; } ]`, but `why ctx { id = "parent"; attr = "derived"; }` ⇒ `[ ]` even though `parent.derived` does read `parent.self-v`. `why ctx { id = "child"; attr = "no-such-attr"; }` ⇒ `[ ]`, and an unknown NODE answers `[ ]` too — that one is the `or { deps = [ ]; }` default on this module's internal `edges` binding, which survived the export's retirement precisely so `why` keeps it. Tests: `test-why`, `test-why-on-an-unknown-node-answers-empty` (`ci/tests/contract.nix`) |
...
| The plane's context is no longer a FIELD of the sealed context, so nothing forces it by accident and nothing hides an absent plane behind laziness either — a caller that wants it composes it and thereby forces it | That composition lived at `ci/tests/resolve.nix` and was the ecosystem's only site forcing `build`; it re-sited to `gen-memo`'s suite, so what stays true here is the seal's shape rather than a figure about a file this repository no longer has. A **cyclic** declared relation still resolves on the cold path, measured at `gen-scope`'s `fold-equations.test-cyclic-declared-edges-leave-the-cold-path-alone` |
...
```json
["_buildSchedule","_scheduleWith","attr","cascade","classKey","materialize","materializeAll","nta","reference","terminalBind","why"]
```
